...
// const object replaces enum — required when erasableSyntaxOnly is enabled
...
export interface CRDTRecord {
    ...
    originLeft: EventId | null;  // eventId of record immediately to the left when inserted. Null if inserted at 0
...
export interface TransformedOp {
    type: 'insert' | 'delete';
    index: number;
    char?: string; // only present for insert
}